 ...
 * {@link com.sohlman.liferay.validate.service.ValidateService} interface.
 * 
 *
 * 
 * <p>
...
public class ValidateServiceImpl extends ValidateServiceBaseImpl {
	 ...
	 * com.sohlman.liferay.validate.service.ValidateServiceUtil} to access the
	 * validate remote service.
	 */
		...
		try {
			...
			for (JournalArticle journalArticle : list) {
				...
				try {
					...
					// Uncomment this if you want to list all articles to be
					// scanned

					// _log.error(String.format("OK Article : groupID:%d articleId:%s version:%f ",
					// journalArticle.getGroupId(),
					...
					_log.error(String
							.format("2ArticleVirhe\tgroupID\t%d\tarticleId\t%s\tversion\t%f\tstructureId\t%s\texception\t%s\tmsg\t%s",
									journalArticle.getGroupId(), journalArticle
											.getArticleId(), journalArticle
											.getVersion(), journalArticle
											.getStructureId(), pe.getClass()
	...
	throws PortalException, SystemException {
		 ...
		 * 
		 * && ((smallImageBytes == null) || (smallImageBytes.length >
		 * smallImageMaxSize))) {
		 * 
		 * 
		 * 
	...
	SystemException {
		...
		for (com.liferay.portlet.dynamicdatamapping.storage.Field field : fields) {
			...
			if (!ddmStructure.hasField(field.getName())) {
				...
				sb.append("\tStorageFieldNameException\t");
			...
			&& (classNameId == JournalArticleConstants.CLASSNAME_ID_DEFAULT)) {
				...
				sb.append("\tStorageFieldRequiredException\t");
		...
		if (sb.length() > 0) {
			...
			throw new StorageFieldNameException(sb.toString());
